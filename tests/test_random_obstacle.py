...
def test_init(random_obstacle_2d : RandomObstacle2D, random_obstacle_3d : RandomObstacle3D):
    assert random_obstacle_2d.obstacle_density == 0.5
    assert random_obstacle_3d.obstacle_density == 0.5

def test_generate_grid(random_obstacle_2d : RandomObstacle2D, random_obstacle_3d : RandomObstacle3D):
...
def test_obstacle_density(random_obstacle_2d : RandomObstacle2D, random_obstacle_3d : RandomObstacle3D):
    grid_2d = random_obstacle_2d.generate_grid()
    obstacle_count = np.count_nonzero(grid_2d == 1)
    assert np.isclose(obstacle_count / (5 * 5), 0.5, atol=0.1)
    grid_3d = random_obstacle_3d.generate_grid()
    obstacle_count = np.count_nonzero(grid_3d == 1)
    assert np.isclose(obstacle_count / (5 * 5 * 5), 0.5, atol=0.1)
...
def test_obstacle_density_edge_cases():
    ...
    grid_2d = grid.generate_grid()
    obstacle_count = np.count_nonzero(grid_2d == 1)
    assert np.isclose(obstacle_count / (5 * 5), 0.9, atol=0.1)
    ...
    grid_3d = grid.generate_grid()
    obstacle_count = np.count_nonzero(grid_3d == 1)
    assert np.isclose(obstacle_count / (5 * 5 * 5), 0.9, atol=0.1)
