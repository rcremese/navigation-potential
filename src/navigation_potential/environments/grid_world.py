...
class GridWorld(abc.ABC):
    def __init__(self, grid_size : int, seed=None):
        ...
        if grid_size <= 1:
            raise ValueError("Grid size must be greater than one in order to place initial and goal positions")
    ...
    def set_obstacle(self, position : tuple[int]):
        ...
        if np.any([pos < 0 for pos in position]) or np.any([pos >= self.grid_size for pos in position]):
